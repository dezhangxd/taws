...
void load_ls_data(kissat *solver, CCAnr *lssolver) {
    ...
    for (clause *c = start; c != end; c = kissat_next_clause(c)) {
        ...
        for (; i < size; i++) {
            ...
            if (f->eliminated) puts("eliminated var");
    ...
    if (TERMINATED(11)) return;
    // printf("c var %d(max: %d) cls %d(binary %d)\n", ls_var, mx_var, ls_cls, binary_size);
...
int kissat_ccanr (kissat *solver) {
    // printf("c LS_RES (%d: %ld conflicts)\n", ++solver->ls_call_num, CONFLICTS);

    // kissat_restart_and_flush_trail(solver);

    // printf("c finish restart\n");
    ...
    // double ls_parse_time = cpuTime() - start;
    ...
    // double ls_used_time = cpuTime() - start;
    // printf("c unsat num: %d -> %d\n", init_unsat_ct, solver->lssolver->best_cost);
    // printf("c time: %.2lf (parse: %.2lf)\n\n", ls_used_time, ls_parse_time);
